...
- **The round page is facts only; the model lives on `/analysis`.** The round-by-round page shows nothing derived from a model — no grid shading, no hotspot rings, no per-death context (decision 2026-09-17: "หน้า demo reviewer ก็คือหน้า demo reviewer"). Everything learned from the professional dataset is on the analysis page, whose first mode **แผนที่ทีมอาชีพ** works without any uploaded match.
- **One grid, three questions (on `/analysis`), one way of drawing.** Every map on the analysis page is the same soft heatmap (weighted circles at cell centres, blurred, continuous ramp, darker = more), with the same zone labels and the same hover: a table row highlights its zone on the map. The 32×32 ML grid is shaded one layer at a time, and the key under the map always states which. **ดวลบ่อย** answers "where do fights happen at all" with the red "both sides" ramp, log-scaled between the least and most fought-over cell, plus numbered pins for the 8 busiest MeanShift hotspots (of 17) — hovering a pin or its table row shows the hotspot's real radius. **CT / T ได้เปรียบ** answers "where does my side usually win": that side's ramp on cells where it wins ≥ 50 % of duels, 50 % palest to 100 % darkest; cells where it wins less stay unpainted, because the other side wins there and that is the other button (no third "avoid" colour). Its table lists the zones where the side wins most (cells grouped by callout, ≥ 30 duels). **ประเภทพื้นที่** shows the three fight types KMeans found as crisp rounded tiles (categories are unordered, so they are not blurred into each other), in the same three colours as `output/grid_ml1_map.png` so the page and the slide figure read as one thing; English type names are glossed in Thai and hovering a type isolates it. Colour is never assigned arbitrarily: ordered values get an ordered ramp, unordered groups get distinct hues, and the two saturated side hues are never reused for anything else. All numbers come from 7,158 duels in 50 professional Mirage matches (`research/grid_ml1.py`, per-cell `ct_win`, `duels`, `cluster` and `place`).
- **Choices explain themselves.** The four analysis modes are cards with a one-line answer each ("ทีมอาชีพดวลกันตรงไหน ใครได้เปรียบตรงไหน", …) so a first-time viewer never has to click to find out; rendering knobs (radius / blur / opacity) are folded away under "ปรับการแสดงผล".
