...
#   python pipeline/round_win.py --source=db     บังคับ DB
#   python pipeline/round_win.py --source=csv    บังคับ csv (Docker ที่ไม่มี DB)
SOURCE = next((a.split("=", 1)[1] for a in sys.argv[1:] if a.startswith("--source=")), "auto")
sys.path.insert(0, str(ROOT))                   # ให้ import pipeline.datasource ได้เมื่อรันเป็นสคริปต์
...
def main() -> None:
    ...
    if "round_winner" not in df.columns:
        sys.exit("ข้อมูลไม่มีคอลัมน์ round_winner — csv รุ่นเก่า รัน python pipeline/demoparser.py --force ก่อน")
