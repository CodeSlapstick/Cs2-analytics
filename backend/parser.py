...
def _round_table(dem) -> pl.DataFrame:
    ...
    if bp is None or not len(bp):
        rounds = rounds.with_columns(pl.lit(None, pl.Float64).alias("bomb_plant_x"),
    ...
    # bomb_site ที่ awpy ส่งมาใช้ไม่ได้ (ลง 'bombsite_b' ทุกแถว) จึงไม่หยิบคอลัมน์นั้นมาเลย
    # ตัดสินจากพิกัดจุดวางแทน — สูตรอยู่ที่ backend/review.py ที่เดียว ให้ parser กับ backfill ตรงกันเสมอ
