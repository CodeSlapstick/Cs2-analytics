...
downsample_ticks.py — ดึงตำแหน่งผู้เล่นจากเดโม แต่เก็บแค่วินาทีละครั้ง (1 Hz)
    ...
    python -m backend.parser.positions              # อ่านทุกไฟล์ที่ยังไม่เคยอ่าน
    python -m backend.parser.positions --limit 1    # ลองไฟล์เดียวก่อน
    python -m backend.parser.positions --hz 2       # เก็บ 2 ครั้งต่อวินาทีแทน
