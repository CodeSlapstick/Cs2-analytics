...
- **เนื้อหารอบ** — ซ้าย: แถวชั้นข้อมูล → แผนที่ → key อธิบายทุกสัญลักษณ์ → แถบเล่นย้อน · ขวา: สองทีม + ไทม์ไลน์
  ...
  **ระบายกริดด้วย** เลือกได้ทีละอย่าง (ตัวเลขทุกชั้นมาจาก grid_ml1 ทั้งดาต้าเซ็ต 50 แมตช์ ตอนนี้มีแค่ Mirage)
  · **ดวลบ่อย** — ไล่เฉดม่วงเฉดเดียว จาง = ดวลน้อย เข้ม = ดวลบ่อย ขั้นทั้งห้าคิดจากควอนไทล์ของช่องในแมพนั้น ไม่ใช่เลขตายตัว (ลำดับอยู่ที่ความสว่าง คนตาบอดสีจึงอ่านได้)
  · **CT / T ได้เปรียบ** — ระบายช่องที่ฝั่งนั้นชนะดวล ≥ 50 / 55 / 65 % เป็นสีฝั่ง 3 ระดับ และแดงเฉพาะช่องที่ฝั่งตรงข้ามชนะ ≥ 65 % — CT กับ T จึงเห็นคนละแผนที่
...
ดูจุดที่ผู้เล่นในแมตช์ของทีมตาย แล้วเทียบกับเดโมทีมอาชีพ 50 แมตช์บนแมพเดียวกัน — สองมุมมอง (ทีมเราตายตรงไหน / เทียบกับทีมอาชีพ)
กรองได้ว่าดูเฉพาะตอนเป็น CT หรือ T และเจาะจงแมตช์เดียวได้ ตารางข้างแผนที่บอกชื่อตำแหน่งในแมพ (BombsiteA, Catwalk, …) ว่าตรงไหนเราตายบ่อยกว่าเขากี่เท่า
สีของแผนที่บอกฝั่งที่เลือกดู (CT ฟ้า · T ส้ม · รวมสองฝั่ง แดง) ไล่ 5 ขั้น จางน้อย เข้มบ่อย · ไม่มีศัพท์ ML บนหน้าจอผู้ใช้ — เรื่องโมเดลอยู่ในเอกสารนี้เท่านั้น
...
state ทั้งหมดอยู่บน URL (`?sb=0` ย่อ sidebar, `board=1` สกอร์บอร์ด, `g=smoke,flash` ชนิดระเบิดที่แสดง (`g=none` = ไม่แสดงเลย), `z=2.5` `c=x,y` ซูม/จุดที่มองอยู่บนแผนที่, `grid=freq|ct|t` ชั้นที่ระบายลงกริด, `hs=1` วงจุดปะทะ, `pb=1` โหมดเล่นย้อน + `t=12.5` วินาทีในรอบที่ดูค้างไว้, `p=` ผู้เล่นที่ไฮไลต์, `d=` การตายที่เลือก)
...
| GET | `/api/review/grid?map=de_mirage` | ช่องกริดเป็นพิกเซลพร้อม `ct_win` / `duels` ต่อช่อง (หน้าเว็บระบายพื้นที่ได้เปรียบแยกฝั่ง) + วงจุดที่ดวลกันบ่อย |
