...
สามโหมด เลือกจากการ์ดบนสุด (แต่ละการ์ดมีคำอธิบายหนึ่งบรรทัดว่าตอบคำถามอะไร) หัวเรื่องเปลี่ยนตามโหมดว่ากำลังดูอะไร
สองโหมดแรกวาดเป็น **heatmap แบบเดียวกัน** (วงถ่วงน้ำหนักที่กึ่งกลางช่องแล้วเบลอ ไล่สีต่อเนื่อง ยิ่งเข้มยิ่งมาก ชี้แถวในตาราง = เน้นโซนบนแผนที่)
...
- **อ่านทางเราออกไหม** — อ่านแมตช์ของทีมทีละรอบว่าเดาไซต์ที่จะเข้าได้ตั้งแต่วินาทีไหน เทียบกับทีมอาชีพ (เบื้องหลังคือ `research/site_ml.py`)

**ผลของโมเดลไม่ขึ้นหน้าจอผู้ใช้ที่ไหนเลย** (ตัดสินใจ 18 ก.ย. 2026 — "เอา ML ไว้หลังบ้าน") `research/grid_ml1.py` ยังเป็นตัวกำหนดว่าชุดอ้างอิงคืออะไรและกริดแบ่งยังไง แต่ไม่มีหน้า ไม่มีปุ่ม และไม่มี endpoint ให้ดูจุดปะทะ/ประเภทพื้นที่/อัตราชนะดวลของโมเดลอีกต่อไป — เรื่องโมเดลอยู่ใน `research/` กับเอกสารนี้เท่านั้น
ภาษาบนหน้าจอ: ห้ามเรียกตัวเลขของโมเดลว่าโอกาสชนะรอบ และไม่มีคำตัดสินผู้เล่น (มีเทสต์สแกนไฟล์ `.tsx` ดักไว้)
...
frontend/src/       หน้าเว็บ — React + TypeScript + Vite + TanStack Query (nginx ใน Docker)
  ...
  AnalysisPage.tsx    เครื่องมือวิเคราะห์: ทีมเราตายตรงไหน · เทียบกับทีมอาชีพ · อ่านทางเราออกไหม
