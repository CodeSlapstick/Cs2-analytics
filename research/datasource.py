# -*- coding: utf-8 -*-
"""
research/datasource.py — แหล่งข้อมูลคิลของโมเดล เลือกได้ว่าอ่านจาก PostgreSQL หรือ csv

    from research.datasource import load_kills
    ...
    จากคอมมานด์ไลน์ของสคริปต์โมเดล:  python research/grid_ml.py --source=db
...
def _from_csv(map_name: str | None) -> pd.DataFrame:
    if not CSV.exists():
        raise FileNotFoundError(f"ไม่พบ {CSV} — รัน python research/demoparser.py ก่อน")
    df = pd.read_csv(CSV)
